...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	b := (runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight /
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	var speedInM = meanSpeed(action, duration) * kmhInMsec
	a := (walkingCaloriesWeightMultiplier*weight +
		((speedInM*speedInM)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	c := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return c
}
